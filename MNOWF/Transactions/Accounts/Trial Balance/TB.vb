    ...
    Sub FILLGRIDOPCLOSING()
        Try
            ...
            If chkdetails.CheckState = CheckState.Checked Then
                ...
                For Each DTROW As DataRow In DT.Rows
                    If (Val(DTROW("DR")) - Val(DTROW("CR"))) > 0 Or (Val(DTROW("DR")) - Val(DTROW("CR"))) < 0 Or Val(DTROW("OPBALDR")) > 0 Or Val(DTROW("OPBALCR")) > 0 Or Val(DTROW("DR")) > 0 Or Val(DTROW("CR")) > 0 Then
                        Dim SPACE As String = SPACER(Val(DTROW("LEVELNO")))
                        If chkopbal.Checked = False Then
                            If DTROW("NAME").ToString.Trim = "" And DTROW("SECONDARY").ToString.Trim = "" And DTROW("UNDER").ToString.Trim = "" And DTROW("GPNAME").ToString.Trim = "" Then
...
#End Region
    ...
    Private Sub gridtrialbalance_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles gridtrialbalance.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then showform(gridtrialbalance.CurrentRow.Cells(GNAME.Index).Value.ToString.Trim)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub

    Private Sub gridopclo_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles gridopclo.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then showform(gridopclo.CurrentRow.Cells(GOPCLNAME.Index).Value.ToString.Trim)
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
