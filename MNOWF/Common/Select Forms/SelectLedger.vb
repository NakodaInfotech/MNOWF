...
Public Class SelectLedger
    ...
    Sub FILLGRID(ByVal WHERECLAUSE As String)
        Try
            ...
            Dim DT As DataTable = OBJCMN.search(" LEDGERS.Acc_cmpname AS LEDGERNAME, LEDGERS.Acc_code AS CODE, GROUPMASTER.GROUP_SECONDARY AS [GROUP], CITYMASTER.city_name as CITY", "", " LEDGERS INNER JOIN GROUPMASTER ON LEDGERS.Acc_cmpid = GROUPMASTER.group_cmpid AND LEDGERS.Acc_locationid = GROUPMASTER.group_locationid AND LEDGERS.Acc_yearid = GROUPMASTER.group_yearid AND LEDGERS.Acc_groupid = GROUPMASTER.group_id LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id AND LEDGERS.Acc_cmpid = CITYMASTER.city_cmpid AND LEDGERS.Acc_locationid = CITYMASTER.city_locationid AND LEDGERS.Acc_yearid = CITYMASTER.city_yearid", STRSEARCH & WHERECLAUSE & " AND ACC_CMPID = " & CmpId & " AND ACC_LOCATIONID = " & Locationid & " AND ACC_YEARID = " & YearId & " order by LEDGERS.Acc_cmpname")
            ...
            If TXTNAME.Text.Trim <> "" And cmbname.Text.Trim <> "" Then
                If rbstart.Checked = True Then
                    If cmbname.Text = "Name" Then
                        FILLGRID(" AND LEDGERS.ACC_CMPNAME = '" & TXTNAME.Text.Trim & "'")
                    ElseIf cmbname.Text = "Code" Then
                        FILLGRID(" AND LEDGERS.ACC_CODE = '" & TXTNAME.Text.Trim & "'")
                    ElseIf cmbname.Text = "Group" Then
                        FILLGRID(" AND GROUPMASTER.GROUP_NAME= '" & TXTNAME.Text.Trim & "'")
